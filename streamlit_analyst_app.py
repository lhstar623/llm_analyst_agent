...
def main():
    ...
    if st.session_state.current_analysis:
        ...
        with tab3:
            ...
            if analysis.image_html_path and len(analysis.image_html_path) > 0:
                # CHANGED: For loop to display all the html images
                for image_html_path in analysis.image_html_path:
                    try: 
                        with open(image_html_path, 'r', encoding='utf-8') as f:
                            html_content = f.read()
                        st.components.v1.html(html_content, height=500, scrolling=True)
                    except Exception as e:
                        st.error(f"Error displaying HTML: {str(e)}")

            elif analysis.image_png_path and len(analysis.image_png_path) > 0:
                # CHANGED: For loop to display all the png images
                for image_png_path in analysis.image_png_path:
                    try:
                        st.image(image_png_path)
                    except Exception as e:
